...
namespace Soyo.SoyoRuntimeConsole.Tests.Editor
{
    ...
    public class ConsoleAttributeScannerTests
    {
        ...
        public void ScanClass_SimpleCommand_FindsOneCommand()
        {
            ...
            var (commands, helpTexts) = ConsoleAttributeScanner.ScanClass(typeof(SimpleCommandFixture));

            Assert.That(commands.Count, Is.EqualTo(1));
            Assert.That(commands[0].CommandName.Name, Is.EqualTo("Hello"));
        ...
        {
            var (commands, helpTexts) = ConsoleAttributeScanner.ScanClass(typeof(ParameterCommandFixture));
        ...
        {
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(ParameterCommandFixture));
        ...
        {
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(CustomParamNameFixture));
        ...
        {
            var (commands, helpTexts) = ConsoleAttributeScanner.ScanClass(typeof(CustomParamNameFixture));

            Assert.That(helpTexts.Count, Is.EqualTo(1));
            Assert.That(helpTexts.ContainsKey(commands[0].CommandName), Is.True);
            Assert.That(helpTexts[commands[0].CommandName], Is.EqualTo("A command with custom parameter names."));
        }
        ...
        {
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(KeyedClassFixture));
        ...
        {
            var (commands, _) = ConsoleAttributeScanner.ScanClass(
            ...
            Assert.That(commands.Any(c => c.CommandName.Name == "class_keyed"), Is.True);
            Assert.That(commands.Any(c => c.CommandName.Name == "global_in_keyed_class"), Is.True);
            Assert.That(commands.Any(c => c.CommandName.Name == "method_keyed"), Is.False);
        }
        ...
        public void ScanClass_TargetKeyDebug_ExcludesAdminKeyedCommand()
        {
            ...
            Assert.That(commands.Count, Is.EqualTo(1));
            Assert.That(commands[0].CommandName.Name, Is.EqualTo("method_keyed"));
        }
        ...
        public void ScanClass_MethodLevelKey_FilteredCorrectly()
        {
            ...
            Assert.That(commands.Count, Is.EqualTo(1));
            Assert.That(commands[0].CommandName.Name, Is.EqualTo("debug_only"));
        }
        ...
        public void ScanClass_MethodKeyOverridesClassKey()
        {
            ...
            Assert.That(commands.Count, Is.EqualTo(3));
            Assert.That(commands.Any(c => c.CommandName.Name == "class_keyed"), Is.True);
            Assert.That(commands.Any(c => c.CommandName.Name == "method_keyed"), Is.True);
            Assert.That(commands.Any(c => c.CommandName.Name == "global_in_keyed_class"), Is.True);
        }
        ...
        public void ScanClass_DefaultParameter_WarnsButDoesNotSkip()
        {
            ...
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(DefaultParamFixture));

            // 默认参数方法不应被跳过
            Assert.That(commands.Count, Is.EqualTo(1));
            Assert.That(commands[0].CommandName.Name, Is.EqualTo("default_param"));
        }
        ...
        public void ScanClass_GenericMethod_SkippedWithWarning()
        {
            ...
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(GenericMethodFixture));
        ...
        {
            var (commands, _) = ConsoleAttributeScanner.ScanClass(typeof(NonCommandFixture));
        ...
        public void EndToEnd_ScannedCommand_ExecutesViaConsoleBase()
        {
            ...
            // 使用 TestConsole（内部类，通过反射或直接使用 ConsoleBase 子类）
            var console = new TestConsole(config);

            // 注意：TestConsole 是 ConsoleBase 的子类，我们需要在 ConsoleTestFixtures 中定义
            // 或者直接使用 Console.Create
